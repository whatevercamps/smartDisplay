...
export class AuthService {
  ...
  registerUser(user) {
    ...
    return this.http.post('http://192.168.10.113:3000/users/register', user, {headers: headers}).pipe(
      map(res => res.json()));
  }
  ...
  getProfile() {
    ...
    headers.append('Content-Type', 'application/json');
    return this.http.get('http://192.168.10.113:3000/users/profile', {headers: headers}).pipe(
      map(res => res.json()));
  }
